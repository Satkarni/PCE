...
void ks0108DrawPageArrow(uint8_t direction)
{
	if (DOWN == direction)
	{
		ks0108DrawLine(116,54,122,54,BLACK);
		ks0108DrawLine(116,54,119,57,BLACK);
		ks0108DrawLine(122,54,119,57,BLACK);
	}
	else if (UP == direction)
	{
		ks0108DrawLine(116,25,122,25,BLACK);
		ks0108DrawLine(116,25,119,22,BLACK);
		ks0108DrawLine(122,25,119,22,BLACK);
	}
}
...
void ks0108Puts(char* str) {
	...
	while(*str != 0) {
		if(*str == '\n') {
			...
			ks0108GotoXY(ks0108Coord.x+SPACE,ks0108Coord.y);
		}
		else {
			ks0108PutChar(*str);
		}
...
void ks0108Puts_mod(char* str,uint8_t x_arg,uint8_t y_arg,uint8_t space)
{
	uint8_t height = ks0108FontRead(ks0108Font+FONT_HEIGHT);
	
	ks0108FillRect(x_arg,y_arg,space,height,WHITE);
	ks0108GotoXY(x_arg,y_arg);
...
void ks0108Puts_mod_align_center(const char* str,uint8_t x_arg1,uint8_t x_arg2,uint8_t y_arg,uint8_t space)
{
	uint8_t height = ks0108FontRead(ks0108Font+FONT_HEIGHT);
	uint16_t swidth = ks0108StringWidth(str);
	
	uint16_t x_start = (x_arg2 - x_arg1)/2;
	x_start = x_arg1 + x_start - (swidth/2);
	ks0108FillRect(x_arg1,y_arg,space,height,WHITE);
	ks0108GotoXY(x_start,y_arg);
...
uint16_t ks0108StringWidth(const char* str) {
	uint16_t width = 0;
...
void ks0108GotoXY(uint8_t x, uint8_t y) {
	...
	if(x > 127) x = 127;								// ensure that coordinates are legal
	if(y > 63)	y = 63; 
...
extern inline void ks0108Enable(void);

extern inline uint8_t ks0108ReadData(void);
...
void ks0108LoadBitmap(const uint8_t *bitmap)
{
	uint16_t i, j,by;
	for(i=0; i<64; i+=8)
	{
