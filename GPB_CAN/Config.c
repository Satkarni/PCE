...
param wt_trip =				{ PARAM_WT_TRIP,  WT_TRIP_DF,94,110,1,0};
...
void config_init()
{
	...
	lop_trip.prm =			(tmp == 0xFFFF) ? lop_trip.dfault : tmp;
	
	tmp = eeprom_read_word((uint16_t*)EEP_LOP_TRIP_DELAY_ADDR);
	lop_trip_delay.prm =	(tmp == 0xFFFF) ? lop_trip_delay.dfault : tmp;
	
	tmp = eeprom_read_word((uint16_t*)EEP_WT_TRIP_ADDR);
	wt_trip.prm =			(tmp == 0xFFFF) ? wt_trip.dfault : tmp;
	
	tmp = eeprom_read_word((uint16_t*)EEP_WT_TRIP_DELAY_ADDR);
	wt_trip_delay.prm =		(tmp == 0xFFFF) ? wt_trip_delay.dfault : tmp;
	
	tmp = eeprom_read_word((uint16_t*)EEP_FLT_EN_ADDR);
	flt_enable.prm =		(tmp == 0xFFFF) ? flt_enable.dfault : tmp;
	
	tmp = eeprom_read_word((uint16_t*)EEP_FLT_EN_DELAY_ADDR);
	flt_enable_delay.prm =	(tmp == 0xFFFF) ? flt_enable_delay.dfault : tmp;
	
	tmp = eeprom_read_word((uint16_t*)EEP_FLT_DIS_ADDR);
	flt_disable.prm =		(tmp == 0xFFFF) ? flt_disable.dfault : tmp;
